...
(* J'ai essayé de faire de mon mieux avec des labels de partout mais il y a une fatal error que je n'explique pas :(. l'encadrant m'a soufflé l'idée d'implémentation suivante :
   ...
   on peut donc enlever sign de labels_____________________________________________________*)
let rec find_deep gre src sink accu = 
    if src = sink then accu else
        try
            let arcs_sortants = (out_arcs gre src) in 
            let rec loop = function 
                | [] -> []
                | (dest, lab) :: r -> if ((lab.current > 0) && (not_visited_node gre dest)) then 
                        let chemin = (find_deep (add_arcs_c (add_arcs_c gre dest src { lab with visited = true }) src dest { lab with visited = true }) dest sink ((src,(dest,lab))::accu) ) in
                            match chemin with
                                  | [] -> loop r
                                  | _ -> chemin
                    else loop r 
            in loop arcs_sortants
        with Not_found -> []

let update_residual_graph lemax g path =
    let rec loop gr path =
        match path with 
        |[] -> gr
        |(id1, (id2, lab)) :: r ->
          let minus_lab = { lab with current = (lab.current - lemax) ; visited = false } in
          let maxi_lab = { lab with current = (lab.current + lemax) ; visited = false } in
          loop (add_arcs_c (add_arcs_c gr id1 id2 minus_lab) id2 id1 maxi_lab) r
    in
    loop g path

let update_graphe_initial gre gri = 
    e_fold gri (fun gr id1 id2 lab -> match find_arc gre id2 id1 with
        | None -> failwith "L'arc " ^ string_of_int id2 ^ "->"^ string_of_int id1 " n'existe pas dans le graphe d'ecart"
        | Some (a, labe) -> new_arc gri id1 id2 labe
        ) (clone_nodes gri)
        
let ford_fulkerson2 gr debut fin =
  let gre =  make_ecart gr in
  let rec loop gr d f =
    let chemin = find_deep d f [] in
    let () =  Printf.printf "________________\n"
    (*let () = print_path chemin in           *)     (*debuggage*)
    match chemin with
        |[] -> update_graphe_initial gre gr
        |_ -> loop (update_residual_graph (max_flow 9999 chemin) gre chemin) d f
  in
  loop gre debut fin
   
(*fin dernière version non testée (ça compile ?)___________________________________________*)
      ...
      try 
        ...
        let rec loop reste =
          ...
          | (id2, lab) :: r -> if ((lab.current > 0) && (not_visited_node gra id2))  then 
              ...
              let chemin = (find_path_ford (add_arcs_c (add_arcs_c gra id2 id { lab with visited = true }) id id2 { lab with visited = true }) id2 idfin ((id,(id2,lab))::accu) ) in
              match chemin with
              | [] -> loop r
              | _ -> chemin
            else loop r 
        ...
        loop out
      with Not_found -> [])
  in find_path_ford gre id idfin accu 
